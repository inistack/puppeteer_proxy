...
(async () => {
    ...
    const proxyServer = new URL(process.env.PROXY_SERVER);
    const myProxyUrl = `http://${username}:${password}@${proxyServer.host}`;


    const server = new Server({
        port: 8000, host: 'localhost', verbose: true, prepareRequestFunction: () => {
            return {
                upstreamProxyUrl: `${myProxyUrl}`
            };
        }
    });

    await server.listen();
    console.log(`Proxy server is listening on port ${server.port} and host ${process.env.PROXY_SERVER}`);
